...
function tfidf(entireString) {
    ...
    var count = entireMatchedWords.reduce(function (stats, word) {
    ...
    //Sort by value
    var sortable = [];
    for (var text in count) {
        sortable.push([text, count[text]]);
    }
    ...
    //Generates Labeled Buttons for all words in array and their number of occurances
    for (var x in sortable) {
        document.getElementById("tfidfBox").innerHTML += "<button id = " + sortable[x][0] + " onclick = input(\"" + sortable[x][0] + "\")>" + sortable[x][0] + "</button";
        document.getElementById("tfidfBox").innerHTML += " " + sortable[x][1] + "<br>";
    }

}

var str = entireString;
var strlength = str.length; 

for (var i = 0; i < keys.length; i++) {
    var word = keys[i];
    var wordobj = counts[word];
    wordobj.tfidf = count * log(strlength / wordobj.df);
  }

var files = ['rainbow.txt', 'sports.txt', 'eclipse.txt', 'fish.txt'];

var strcount = entireString

var count = Object.keys(strcount).length;
console.log(count);
